...
def create_autorefresh_component(interval=1000):
    """
    Create a custom component that will auto-refresh the page at specified intervals.
    """
    html_code = f"""
    <script>
        // Auto-refresh the page every {interval} milliseconds
        setTimeout(function() {{
...
def display_eye_exercise_timer():
    """
    Display the eye exercise timer with auto-refresh.
    """
    if 'show_exercise_timer' in st.session_state and st.session_state.show_exercise_timer:
        # Calculate elapsed time
        elapsed_time = (datetime.now() - st.session_state.exercise_start_time).total_seconds()
        exercise_duration = st.session_state.active_exercise['duration_seconds']
        remaining_time = max(0, exercise_duration - elapsed_time)
        
        # Display progress and timer
        st.progress(min(1.0, elapsed_time / exercise_duration))
        st.metric("Time Remaining", f"{remaining_time:.1f} sec")
        st.markdown(f"**Instructions:** {st.session_state.active_exercise['description']}")
        
        # Auto-refresh every second
        st.components.v1.html(create_autorefresh_component(interval=1000), height=0)
...
def display_eye_break_timer():
    """
    Display the eye break timer with auto-refresh.
    """
    if 'eye_timer_active' in st.session_state and st.session_state.eye_timer_active:
        # Calculate elapsed time
        elapsed_time = (datetime.now() - st.session_state.eye_timer_start).total_seconds()
        interval_minutes = st.session_state.eye_care_routine["base_routine"]["interval_minutes"]
        remaining_time = max(0, interval_minutes * 60 - elapsed_time)
        
        # Display progress and timer
        st.progress(min(1.0, elapsed_time / (interval_minutes * 60)))
        st.metric("Next eye break in", f"{remaining_time // 60:.0f}:{remaining_time % 60:02.0f}")
        ...
        # Check if the timer has ended
        if remaining_time <= 0:
            st.success(f"Time for an eye break! Look 20 feet away for {st.session_state.eye_care_routine['base_routine']['duration_seconds']} seconds.")
            if st.button("Reset Timer"):
                st.session_state.eye_timer_start = datetime.now()
                st.rerun()

def custom_track_screen_time():
    """
    Wrapper function for track_screen_time that handles custom duration from user input
    """
    custom_duration = st.session_state.get('custom_duration', 60)
    with st.spinner(f"Tracking your screen time for {custom_duration} seconds..."):
        screen_time_data = track_screen_time(duration=custom_duration)
...
def main():
    ...
    if 'eye_timer_active' in st.session_state and st.session_state.eye_timer_active:
        st.components.v1.html(create_autorefresh_component(), height=0)
    ...
    with tabs[2]:  # Eye Care Tab
        ...
        if 'eye_timer_active' in st.session_state and st.session_state.eye_timer_active:
            display_eye_break_timer()
        if 'show_exercise_timer' in st.session_state and st.session_state.show_exercise_timer:
            display_eye_exercise_timer()
